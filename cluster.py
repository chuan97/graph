...
def recursive_cluster(A, checked=set(), clusters=[{0}]):
    ...
    """
    connected_nodes = set() # empty set, to contain new connections of the current cluster
    for idx in clusters[-1]: # run over current members of current cluster
        if idx not in checked: # if the node hasn't been checked before
            connected_nodes.update(np.nonzero(A[idx, :])[0]) # add its connections to the list of new connections
    ...
    if not remaining: # if all nodes have been checked, nothing more to do, end of recursive depth
        ...
        return recursive_cluster(A, checked=checked, clusters=clusters) # continue filling the current cluster recursively
